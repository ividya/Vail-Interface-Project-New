...
- (void)replyToHTTPRequest
{
    ...
    NSLog(@"uri: %@", uri);
    // Get Parameters 
    NSDictionary *allParams = [self parseGetParams];
    for (id key in allParams)
    {
        NSLog(@"Key: %@, value: %@", key, [allParams objectForKey:key]);
    }
    
    [[InterfaceVariableManager sharedManager] updateInformationWith:uri params:allParams];
